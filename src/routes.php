...
$app->group('', function() use($app) {
    ...
    $app->get('/', function (Request $request, Response $response, $args) {
        $stmt = $this->db->prepare('SELECT count(*) AS res FROM colonist');
        $stmt->execute();
        $data['colonists'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM colonist WHERE gender = "male"');
        $stmt->execute();
        $data['males'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM colonist WHERE gender = "female"');
        $stmt->execute();
        $data['females'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM colonist WHERE id_habitat = NULL');
        $stmt->execute();
        $data['homeless'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM habitat');
        $stmt->execute();
        $data['habitats'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM habitat LEFT JOIN (SELECT count(*) AS number_of_colonists, id_habitat AS idh FROM colonist GROUP BY id_habitat) c ON habitat.id_habitat = c.idh
        WHERE number_of_colonists = size');
        $stmt->execute();
        $data['habitats_full'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM droid WHERE (SELECT count(*) FROM colonist WHERE colonist.id_droid = droid.id_droid) = 0');
        $stmt->execute();
        $data['droids_available'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT count(*) AS res FROM droid WHERE (SELECT count(*) FROM colonist WHERE colonist.id_droid = droid.id_droid) != 0');
        $stmt->execute();
        $data['droids_assigned'] = $stmt->fetch();

        $stmt = $this->db->prepare('SELECT MAX(price) AS max FROM droid');
        $stmt->execute();
        $data['droids_maxPrice'] = $stmt->fetch();
    ...
    $app->get('/colonist/list/all', function (Request $request, Response $response, $args) {
        ...
        if (empty($params['search'])) {
            $stmt = $this->db->prepare('SELECT * FROM colonist LEFT JOIN habitat ON colonist.id_habitat = habitat.id_habitat');
            $stmt->execute();
    ...
    // check if logged user is majordom or majorplanet
    $app->group('', function() use($app) {
        // list habitats
        $app->get('/habitat/list', function (Request $request, Response $response, $args) {
            ...
            if (empty($params['search'])) {
                $stmt = $this->db->prepare('SELECT * FROM habitat 
                                            ...
                                            LEFT JOIN (SELECT firstname AS majordomf, lastname AS majordoml, id_habitat AS idh FROM colonist WHERE authorization = :auth) a 
        ...
        $app->post('/habitat/{id}/info/edit', function (Request $request, Response $response, $args) {
            ...
            // remove current majordom
            $stmt = $this->db->prepare('UPDATE colonist SET authorization = "colonist"
                                        WHERE authorization ="majordom" AND id_habitat = :idh');
            $stmt->bindValue(':idh', $args['id']);
            $stmt->execute();
        ...
        // habitat map
        $app->get('/habitat/map', function (Request $request, Response $response, $args) {
            $stmt = $this->db->prepare('SELECT * FROM habitat');
            $stmt->execute();
            $data['habitats'] = $stmt->fetchall();
            return $this->view->render($response, 'habitat_map.latte', $data);
        })->setName('habitats_map');

    })->add(function($request, $response, $next) {
        if ($_SESSION['logged_colonist']['authorization'] != 'colonist') {
    ...
    if (!empty($_SESSION['logged_colonist'])) {
        return $next($request, $response);
    } else {
